CREATE VIEW IF NOT EXISTS v_refund_risk AS
SELECT * FROM (
    WITH dup AS (
    ...
    flags AS (
        SELECT
            t.ReturnID, t.SSN, t.TaxYear, t.ReportedIncome, t.ClaimedDeductions, t.TaxWithheld, t.RefundRequested,
    ...
    scored AS (
        SELECT
            *,
            (flag_duplicate_ssn*50
            + flag_excessive_deductions*30
            + flag_inflated_refund*25
            + flag_low_income_high_refund*15
            + flag_missing_withholding*10) AS RiskScore
        FROM flags
    )
    SELECT *,
    CASE
